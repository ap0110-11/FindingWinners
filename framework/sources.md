...
- **Reverse-engineer the price:** roughly what revenue growth and margin does the current price imply over the next 3–5 years? Then state whether the actual trajectory beats or misses that implied path. This one step converts a vague "it's cheap" into a testable claim, and it is the core of the whole framework.
  - **Exception — asset-yield businesses.** Growth-and-margin reverse-engineering is the wrong instrument for a company whose asset is contracted, long-lived and leased rather than sold. For powered land, grid interconnection and shell — where revenue lands under ASC 842 as lease income against a 20–40 year asset — reverse-engineer the **implied cap rate and the implied $/MW** instead, and use `framework/valuing-powered-capacity.md`. Applying a revenue-CAGR baseline to a landlord, or a cap rate to a compute reseller, is wrong by multiples rather than percentages. **Check which standard the revenue sits under before choosing the method.**
