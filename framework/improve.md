...
| 2026-09-15 | `framework/valuing-powered-capacity.md` | New file: vertical playbook for powered land, grid interconnection, and shell. Separates ASC 842 landlords from ASC 606 compute resellers, normalizes megawatts to critical IT load, and replaces the multiple with a four-part sum of the parts. Cross-referenced from `framework/sources.md` Lane C as an explicit exception to revenue-CAGR reverse-engineering | The watchlist added GLXY, HUT, CIFR and APLD. The generic Lane C instrument — reverse-engineer implied revenue growth and margin — is the wrong tool for an asset that is leased rather than sold, and applying it to a landlord (or a cap rate to a compute reseller) is wrong by multiples rather than percentages | Pending |
| 2026-09-15 | `framework/valuing-powered-capacity.md` §3 | Added the legal-wrapper subsection: REIT versus C-corp, the gross-income and asset tests, the taxable-REIT-subsidiary cap, impermissible tenant service income, and the interaction between the 90% distribution requirement and a mid-build balance sheet. Evidence bar extended to 12 items | Found while adding APLD, which has publicly pursued REIT treatment. The Part 1 method capitalizes NOI at a cap rate drawn from REIT comps, so applying it to a C-corp's pre-tax NOI overvalued the asset by the entire corporate tax wedge — a defect in the method as originally written, not a company-specific note | Pending |
| 2026-09-15 | `state/watchlist.md` | Recorded that the powered-capacity bucket is **not** regulatorily or structurally homogeneous: APLD's footprint is largely MISO/SPP rather than ERCOT, and it carries a live ASC 606 cloud segment, so it does not shed GPU-obsolescence risk the way the other three do | The theme row originally leaned on ERCOT-specific events (the large-load approval amendment, the Texas queue audit) to argue queue scarcity for the whole bucket. That argument does not transfer outside ERCOT, and treating four names as one position would have mis-stated both the correlation and the sizing | Pending |
